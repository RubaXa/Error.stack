var errorStack = (function (exports) {
	...
	*/
	...
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.aggregation = exports.parseLegacyLogEntry = exports.parseLegacyLog = void 0;

	function parseLegacyLog(log) {
	    return !log ? [] : log.trim().split('\n').map(parseLegacyLogEntry);
	}
	exports.parseLegacyLog = parseLegacyLog;
	function parseLegacyLogEntry(raw) {
	    try {
	        var rawEntry = JSON.parse(raw.trim());
	        var msg = JSON.parse(rawEntry.msg);
	        return __assign(__assign({}, rawEntry), { msg: __assign(__assign({}, msg), { err: __assign(__assign({}, msg.err), { parsedStack: stack.parseStack(msg.err.stack) }) }) });
	    }
	    catch (error) {
	        return {
	            raw: raw,
	...
	exports.parseLegacyLogEntry = parseLegacyLogEntry;
	function aggregation(log, opts) {
	    var url = opts.url, group = opts.group;
	    var grepTest = new RegExp("" + opts.grep, 'i');
	    var filterBy = function (type, val) {
	        if (!opts.grep || !opts.grepBy || !opts.grepBy.includes(type)) {
	            return true;
	        }
	        return val == null ? false : grepTest.test(val);
	    };
	    var aggr = log.reduce(function (agr, entry) {
	        var _a, _b;
	        if ('error' in entry) {
	            return agr;
	        }
	        var checked = (!opts.category || !opts.category.includes(entry.msg.category)) &&
	            (filterBy('message', entry.msg.err.message) ||
	                filterBy('source', entry.msg.err.source) ||
	                filterBy('stack', entry.msg.err.stack) ||
	                filterBy('fn', (_b = (_a = entry.msg.err.parsedStack) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.raw) ||
	                filterBy('url', entry.msg.err.url) ||
	                filterBy('ua', entry.ua));
	        var groupKey = entry.msg.err.message;
	        var fileKey = entry.msg.err.source.split('?')[0] + ":" + entry.msg.err.line + ":" + entry.msg.err.col;
	        if (!checked) {
	            return agr;
	        }
	        if (group === 'ip') {
	            ...
	            groupKey = entry.ts.toString();
	        }
	        else if (group === 'ua') {
	            groupKey = entry.ua;
	        }
	        else if (group === 'email') {
	            groupKey = entry.email || '<<no email>>';
	        }
	        if (url) {
	            fileKey += entry.msg.err.url;
	        }
	        if (opts.detail && entry.msg.err.detail) {
	            fileKey += JSON.stringify(entry.msg.err);
	        }
	        var groupByMsg = agr[groupKey];
	        if (!groupByMsg) {
	            groupByMsg = {
	                count: 0,
	                uniq: {
	            ...
	            agr[groupKey] = groupByMsg;
	        }
	        if (groupByMsg.uniq.mem[entry.ua] !== 1) {
	            groupByMsg.uniq.mem[entry.ua] = 1;
	            groupByMsg.uniq.ua++;
	        }
	        if (!entry.email) {
	            groupByMsg.uniq.anon++;
	        }
	        if (entry.email && groupByMsg.uniq.mem[entry.email] !== 1) {
	            groupByMsg.uniq.mem[entry.email] = 1;
	            groupByMsg.uniq.email++;
	        }
	        if (groupByMsg.uniq.mem[entry.ip] !== 1) {
	            groupByMsg.uniq.mem[entry.ip] = 1;
	            groupByMsg.uniq.ip++;
	        }
	        groupByMsg.count++;
	        var groupByEntry = groupByMsg.detail[fileKey];
	        if (!groupByEntry) {
	            groupByEntry = {
	                count: 0,
	                entry: entry,
	            };
	            groupByMsg.detail[fileKey] = groupByEntry;
	        }
	        groupByEntry.count++;
	        return agr;
	    }, {});
	    return Object.entries(aggr).sort(function (a, b) { return b[1].count - a[1].count; });
	}
	...
	unwrapExports(log);
	var log_1 = log.aggregation;
	var log_2 = log.parseLegacyLogEntry;
	var log_3 = log.parseLegacyLog;

	/* eslint max-lines-per-function:"off", @typescript-eslint/no-unused-expressions:"off" */
	var GROUP_BY = ['message', 'ua', 'ip', 'email', 'url'];
	function ErrorList(props) {
	    var activeEntryRef = react.useRef(null);
	    var visitedEntries = react.useRef(new Set());
	    var _a = react.useState(null), activeEntry = _a[0], setActiveEntry = _a[1];
	    var _b = react.useState('message'), groupBy = _b[0], setGroupBy = _b[1];
	    var _c = react.useState(true), withStack = _c[0], setWithStack = _c[1];
	    var _d = react.useState(false), withUrl = _d[0], setWithUrl = _d[1];
	    var _e = react.useState(false), withDetail = _e[0], setWithDetail = _e[1];
	    var _f = react.useState([]), aggr = _f[0], setAggrLog = _f[1];
	    var selectEntry = react.useCallback(function (entry) {
	        entry && visitedEntries.current.add(entry);
	        setActiveEntry(entry);
	        props.onSelect(entry);
	    }, [setActiveEntry]);
	    var handleNextError = react.useCallback(function () {
	        var idx = aggr.findIndex(function (entry) { return entry[1] === activeEntry; });
	        selectEntry(aggr[idx + 1] ? aggr[idx + 1][1] : aggr[0][1]);
	    }, [activeEntry, setActiveEntry]);
	    react.useEffect(function () {
	        var aggr = log_1(props.entries, {
	            group: groupBy,
	            stack: withStack,
	            url: withUrl,
	            detail: withDetail,
	    ...
	    return (react.createElement(react.Fragment, null,
	        !activeEntry && (react.createElement("div", { className: "error-list-toolbar" },
	            react.createElement("label", { className: "error-list-toolbar-item" },
	                "Group by:",
	                react.createElement("select", { value: groupBy, onChange: function (evt) {
	                        setGroupBy(evt.currentTarget.value);
	                    } }, GROUP_BY.map(function (v) { return (react.createElement("option", { key: v }, v)); }))),
	            react.createElement("label", { className: "error-list-toolbar-item" },
	                "Stack:",
	                react.createElement("input", { checked: withStack, type: "checkbox", onChange: function () {
	                        setWithStack(!withStack);
	                    } })),
	            react.createElement("label", { className: "error-list-toolbar-item" },
	                "URL:",
	                react.createElement("input", { checked: withUrl, type: "checkbox", onChange: function () {
	                        setWithUrl(!withUrl);
	                    } })),
	            react.createElement("label", { className: "error-list-toolbar-item" },
	                "Detail:",
	                react.createElement("input", { checked: withDetail, type: "checkbox", onChange: function () {
	                        setWithDetail(!withDetail);
	                    } })))),
	        react.createElement("div", { className: "" + (activeEntry && 'error-list-slider') },
	            activeEntry && (react.createElement("div", { className: "error-list-nav-ctrl home", title: "List", onClick: function () {
	                    selectEntry(null);
	                } }, "\u21A5")),
	            react.createElement("div", { className: "error-list" }, aggr.map(function (_a, idx) {
	                var message = _a[0], record = _a[1];
	                var uniq = record.uniq;
	                var uniqErr = uniq.ip === 1;
	                var ts = null;
	                var email = null;
	                var ip = null;
	                var msg = message;
	                if (groupBy !== 'message' || withUrl || uniqErr) {
	                    var entry = Object.values(record.detail)[0].entry;
	                ...
	                return (react.createElement("div", { key: idx, ref: activeEntry === record ? activeEntryRef : null, className: "\n\t\t\t\t\t\t\terror-list-entry\n\t\t\t\t\t\t\t" + (visitedEntries.current.has(record) && 'visited') + "\n\t\t\t\t\t\t\t" + (activeEntry === record && 'active') + "\n\t\t\t\t\t\t", onClick: function () {
	                        selectEntry(record);
	                    } },
	                    react.createElement("div", { className: "error-list-entry-info" },
	                        react.createElement("div", { className: "error-list-entry-count" }, record.count),
	                        react.createElement("div", { className: "error-list-entry-msg" }, msg),
	                        ip && react.createElement("div", { className: "error-list-entry-ip" }, ip),
	                        email && react.createElement("div", { className: "error-list-entry-email arr-sep" }, email),
	                        ts && react.createElement("div", { className: "error-list-entry-email arr-sep" }, ts),
	                        !uniqErr && groupBy === 'message' && (react.createElement("div", { className: "error-list-entry-uniq" },
	                            "(ip: ",
	                            uniq.ip,
	                            ", email: ",
	                            uniq.email,
	            ...
	            activeEntry && (react.createElement("div", { onClick: handleNextError, className: "error-list-nav-ctrl next", title: "Next Error" }, "\u27A1")))));
	}
	function renderDetail(detail, url, stack) {
	    return Object.values(detail)
	        .sort(function (a, b) { return b.count - a.count; })
	        .map(function (_a, idx) {
	        var entry = _a.entry, count = _a.count;
	        var err = entry.msg.err;
	        var row = err.parsedStack[0] || {
	            file: err.source,
	            line: err.line,
	            column: err.col,
	        };
	        return (react.createElement("div", { className: "error-list-entry-sub", key: idx },
	            react.createElement("div", { className: "flex" },
	                react.createElement("div", { className: "error-list-entry-count" }, count),
	                react.createElement("div", { className: "error-list-entry-cetegory" }, entry.msg.category),
	                react.createElement("div", { className: "error-list-entry-fn arr-sep" }, row.fn),
	                react.createElement("div", { className: "error-list-entry-file arr-sep" },
	                    row.file.split('?')[0],
	                    ":",
	                    row.line,
	                    ":",
	                    row.column)),
	            react.createElement("div", { className: "error-list-entry-ua" }, entry.ua),
	            url && react.createElement("div", { className: "error-list-entry-url" }, entry.msg.err.url),
	            stack && react.createElement("div", { className: "error-list-entry-stack" }, entry.msg.err.stack)));
	    });
	}
	...
	**/
	...
	function Stack(props) {
	    ...
	    if (!error || !canInspected(error)) {
	        return (react.createElement("div", { className: "stack" },
	            react.createElement("div", { className: "stack-unavailable" },
	                react.createElement("h3", null, "Unavailable to browse source."),
	                error && react.createElement("div", null, error.source))));
	    }
	    return (react.createElement("div", { className: "stack" }, error.parsedStack.map(function (frame, i) { return (react.createElement(StackFrame, { key: i, err: error, frame: frame, prevFrame: error.parsedStack[i - 1], defaultActive: i === 0 })); })));
	}
	function StackFrame(props) {
	    var frame = props.frame, defaultActive = props.defaultActive;
	    var disabled = !(frame && frame.line && /^(https?:)?\//.test(frame.file));
	    var _a = react.useState(defaultActive), active = _a[0], toggleActive = _a[1];
	    return (react.createElement("div", { className: "stack-frame " + (active && 'active'), "aria-disabled": disabled },
	        react.createElement("div", { className: "stack-frame-info flex", onClick: function () {
	                toggleActive(!active);
	            } },
	            react.createElement("div", { className: "error-list-entry-fn" }, frame.fn),
	            react.createElement("div", { className: "arr-sep" }),
	            frame.file,
	            ":",
	            frame.line,
	            ":",
	            frame.column),
	        active &&
	            (disabled ? (react.createElement("div", { className: "stack-unavailable" },
	                react.createElement("h3", null, "Unavailable to browse source."),
	                react.createElement("div", null, frame.raw))) : (react.createElement(StackFrameSource, __assign({}, props))))));
	}
	function StackFrameSource(_a) {
	    var frame = _a.frame, err = _a.err, prevFrame = _a.prevFrame;
	    var source = useSource(frame.file);
	    var sourceRef = react.useRef(null);
	    var html = (source.ok && normSource(source.raw || '', err, frame, prevFrame)) || '';
	    react.useEffect(function () {
	        var _a;
	    ...
	    return (react.createElement("div", { className: "stack-frame-source", ref: sourceRef },
	        source.unavailable && (react.createElement("div", { className: "source-unavailable" },
	            react.createElement("h3", null, "Unavailable to browse source."),
	            react.createElement("div", null, frame.file))),
	        source.ok && react.createElement("pre", { dangerouslySetInnerHTML: { __html: html } }),
	        source.loading && (react.createElement("div", { className: "stack-frame-source-loading" },
	...
	function Tabs(props) {
	    var _a = react.useState(0), idx = _a[0], setActiveIdx = _a[1];
	    return (react.createElement("div", { className: "tabs" },
	        react.createElement("div", { className: "tabs-list" }, props.data.map(function (tab, i) { return (react.createElement("div", { key: i, className: "tab-title " + (i === idx), onClick: function () {
	                setActiveIdx(i);
	            } },
	            react.createElement("span", null, tab.title),
	            tab.badge && react.createElement("div", { className: "tab-badge" }, tab.badge))); })),
	        react.createElement("div", { className: "tab-content" }, props.data[idx].children)));
	}

	function Inspector(props) {
	    var entries = props.entries;
	    var _a = react.useState(null), activeEntry = _a[0], setActiveEntry = _a[1];
	    var records = Object.values((activeEntry === null || activeEntry === void 0 ? void 0 : activeEntry.detail) || {}).sort(function (a, b) { return b.count - a.count; });
	    var _b = react.useState(null), activeRecordEntry = _b[0], setActiveRecordEntry = _b[1];
	    react.useEffect(function () {
	        setActiveRecordEntry(records[0] ? records[0].entry : null);
	    }, [activeEntry]);
	    return (react.createElement("div", { className: "inspector" },
	        react.createElement(ErrorList, { entries: entries, onSelect: setActiveEntry }),
	        activeEntry && (react.createElement("div", { className: "inspector-main" },
	            react.createElement("div", { className: "inspector-stack" },
	                react.createElement(Stack, { key: activeRecordEntry === null || activeRecordEntry === void 0 ? void 0 : activeRecordEntry.msg.err.message, error: activeRecordEntry === null || activeRecordEntry === void 0 ? void 0 : activeRecordEntry.msg.err })),
	            react.createElement("div", { className: "inspector-detail" },
	                react.createElement(Tabs, { data: [
	                        {
	                            title: 'Source',
	...
	function getTabSource(entries, active, onClick) {
	    return (react.createElement("div", { className: "tab-list" }, entries.map(function (rec) {
	        var _a;
	        var err = rec.entry.msg.err;
	        var disabled = !canInspected(err);
	        return listItem(react.createElement("div", { className: "flex" },
	            react.createElement("div", { className: "error-list-entry-fn" }, ((_a = err.parsedStack[0]) === null || _a === void 0 ? void 0 : _a.fn) || '<<unknown>>'),
	            react.createElement("div", { className: "arr-sep" }),
	            err.source,
	            ":",
	            err.line), rec.count, disabled
	            ? false
	            : function () {
	                onClick(rec.entry);
	            }, active === rec.entry);
	...
	function getTabUserAgent(entry) {
	    return react.createElement("div", { className: "tab-list" }, getTabList(entry, /^Mozilla\//i));
	}
	function getTabIP(entry) {
	    return react.createElement("div", { className: "tab-list" }, getTabList(entry, /^\d+\.\d+\./i));
	}
	function getTabEmail(entry) {
	    return (react.createElement("div", { className: "tab-list" },
	        getTabList(entry, /@.+\.[a-z]{2,}$/i),
	        listItem('Anonymous', entry.uniq.anon)));
	}
	function getTabList(entry, filter) {
	    return Object.entries(entry.uniq.mem)
	        .sort(function (a, b) { return (b[1] || 0) - (a[1] || 0); })
	        .map(function (_a) {
	        var key = _a[0], count = _a[1];
	        return (filter.test(key) ? listItem(key, count) : null);
	    });
	}
	function listItem(text, count, onClick, active) {
	    return (react.createElement("div", { onClick: onClick === false ? undefined : onClick, "aria-disabled": onClick === false, "aria-selected": !!active, className: "tab-list-item " + (onClick && 'clickable') },
	        react.createElement("div", { className: "tab-list-item-count" }, count),
	        react.createElement("div", { className: "tab-list-item-text" }, text)));
	}
	...
	function App() {
	    ...
	    var _a = react.useState([]), logEntries = _a[0], setLogEntries = _a[1];
	    var handlePaste = react.useCallback(function (evt) {
	        var raw = evt.clipboardData.getData('text');
	    ...
	    return (react.createElement(react.Fragment, null,
	        react.createElement(DragAndDropZone, { filter: /\.(txt|log)/, onDrop: function (files) {
	                readFilesAsLog(files).then(function (raw) {
	                    setLogEntries(parseLog(raw));
	                });
	            } }),
	        logEntries.length ? react.createElement(Inspector, { entries: logEntries }) : react.createElement(ErrorLog, { onPaste: handlePaste })));
	}
